...
function InputGroupAddon({ className, align = "inline-start", ...props }: ComponentProps<"div"> & VariantProps<typeof inputGroupAddonVariants>) {
  return (
    ...
    <div role="group" data-slot="input-group-addon" data-align={align}
         className={cn('k-input-group__addon', inputGroupAddonVariants({ align }), className)}
         onClick={(e) => {
...
function InputGroupButton({ className, type = "button", variant = "ghost", size = "xs", ...props }: Omit<ComponentProps<typeof KButton>, "size"> &
  ...
  return (
    <KButton
      ...
      data-size={size}
      variant={variant}
      className={cn('k-input-group__button', inputGroupButtonVariants({ size }), className)}
      {...props}
    />
  )
}
