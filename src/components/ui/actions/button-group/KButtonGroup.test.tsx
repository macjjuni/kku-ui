...
describe('KSwitch', () => {
  ...
  it('기본 텍스트와 함께 정상적으로 렌더링된다', () => {
    render(<KButtonGroup>{labelText}</KButtonGroup>);
  ...
  it('id, className 등 기본 HTML 속성이 정상적으로 적용된다', () => {
    ...
    render(<KButtonGroup id={testId} className={testClass}>{labelText}</KButtonGroup>);
  ...
  it('disabled 상태일 때 클릭이 되지 않으며 속성이 적용된다', async () => {
    ...
    render(<KButtonGroup disabled onClick={mockFn}>{labelText}</KButtonGroup>);
  ...
  it('클릭 시 onClick 핸들러가 호출된다', async () => {
    const user = userEvent.setup();
    render(<KButtonGroup onClick={mockFn}>{labelText}</KButtonGroup>);
    const button = screen.getByRole('button');
  ...
  it('primary variant 적용 시 관련 클래스가 포함된다', () => {
    render(<KButtonGroup variant="primary">{labelText}</KButtonGroup>);
    const button = screen.getByRole('button');
    expect(button).toHaveClass('bg-primary');
  });

  it('outline variant 적용 시 관련 클래스가 포함된다', () => {
    render(<KButtonGroup variant="outline">{labelText}</KButtonGroup>);
    const button = screen.getByRole('button');
  ...
  it('sm size 적용 시 높이 관련 클래스가 포함된다', () => {
    render(<KButtonGroup size="sm">{labelText}</KButtonGroup>);
    const button = screen.getByRole('button');
  ...
  it('ref를 통해 DOM 요소에 접근하여 focus가 가능하다', () => {
    ...
    render(<KButtonGroup ref={ref}>{labelText}</KButtonGroup>);
